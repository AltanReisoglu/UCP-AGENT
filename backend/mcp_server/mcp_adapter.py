    ...
    async def _load_all_tools(self):
        ...
        for name, server in self.discovery.list_servers().items():
            ...
            # ADDED: Specific error handling for different types of connection failures
                ...
                print(f"[bold red]Timeout loading tools from server '{name}' after {timeout_secs}s (skipping)[/bold red]")
            except ConnectionError as e:
                print(f"[bold red]Connection error loading tools from server '{name}': {e} (skipping)[/bold red]")
            except Exception as e:
                # Provide clearer output for connection-related errors (httpx, httpcore, etc.)
                print(f"[bold red]Error loading tools from server '{name}': {type(e).__name__}: {e} (skipping)[/bold red]")
...
if __name__ == "__main__":
    # Asenkron işlemleri yürütecek ana fonksiyon
    async def main():
        mcp_connector = MCPConnector()
        
        # 'await' kullanarak fonksiyonun tamamlanmasını bekliyoruz
        tools = await mcp_connector.get_tools()
        
        print(tools)

    # Event loop (olay döngüsü) başlatılıyor
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        # Ctrl+C ile durdurulursa temiz çıkış yap
        pass